...
ВАЖНО. Документация MAX обновляется. Перед сдачей и перед пилотом сверяйте:
  * BASE_URL и способ передачи токена;
  * названия типов обновлений (UPDATE_*);
  * тип кнопки запуска мини-приложения (MINIAPP_BUTTON_TYPE).
Значения вынесены в константы и переменные окружения именно для этого.
Кнопка-ссылка (`link`) работает как запасной вариант, если тип кнопки
запуска мини-приложения в текущей версии платформы называется иначе.
"""
...
def button_miniapp(text: str, url: str) -> dict:
    """Кнопка запуска мини-приложения.

    По умолчанию отправляется обычная ссылка: она открывается и в мобильной,
    и в веб-версии MAX. Если в текущей версии платформы доступен отдельный
    тип кнопки запуска мини-приложения, его имя задаётся переменной
    MINIAPP_BUTTON_TYPE (тогда ссылка кладётся в поле `url` того же формата).
    """
    kind = os.getenv("MINIAPP_BUTTON_TYPE", "link")
    if kind == "link":
        return button_link(text, url)
    return {"type": kind, "text": text, "url": url}
