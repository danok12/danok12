...
ОГРАНИЧЕНИЕ MVP: разбор параметров запуска мини-приложения MAX здесь не
реализован — из среды разработки нет доступа ни к платформе, ни к её
документации. Это единственное место, которое нужно дополнить при переходе на
платформенную кнопку open_app: там ссылки с токеном нет, и пользователя надо
опознавать по параметрам запуска MAX (библиотека MAX Bridge), сверяя их
подпись, а выданный здесь токен оставить для последующих вызовов.
"""
