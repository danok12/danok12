...
# ------------------------------------------------------------- рис. 1.5
def fig15():
    """Угловые деформации сразу на трёх плоскостях, пространственно.

    Каждая видимая грань элемента лежит в своей координатной плоскости
    и показывает изменение своего прямого угла:
      верхняя грань  (плоскость xOy) -> γxy,
      правая грань   (плоскость yOz) -> γyz,
      передняя грань (плоскость xOz) -> γzx.
    """
    w, h, mid = 640, 520, "a15"
    iso = Iso(320, 215, 54)
    K = 200.0                      # условное увеличение углов
    s = HEAD.format(w=w, h=h, mid=mid,
                    alt="Относительные угловые деформации на трёх плоскостях")
    o = (-3.9, 1.3, -3.2)
    for d, name in (((1.0, 0, 0), "x"), ((0, 1.0, 0), "y"), ((0, 0, 1.0), "z")):
        s += ar(iso.p(o), iso.p(add(o, d)), "ax", mid)
        q = iso.p(add(o, mul(d, 1.3)))
        s += txt(q[0], q[1] + 4, name, "ax-lbl")

    s += draw_box(iso, IDENT, "elem-lite", "hid")
    Fm = [[1.0, K * GXY / 2, K * GZX / 2],
          [K * GXY / 2, 1.0, K * GYZ / 2],
          [K * GZX / 2, K * GYZ / 2, 1.0]]
    s += draw_box(iso, Fm, "deformed")

    # рёбра, образующие прямой угол на каждой видимой грани: до и после
    marks = [((0, 0, 1), (1, 0, 0), (0, 1, 0), GXY, "xy", "xOy",
              (320, 62), "middle", (320, 90), (320, 158)),
             ((1, 0, 0), (0, 1, 0), (0, 0, 1), GYZ, "yz", "yOz",
              (446, 182), "start", (442, 202), (380, 212)),
             ((0, 1, 0), (1, 0, 0), (0, 0, 1), GZX, "zx", "xOz",
              (196, 332), "end", (202, 326), (284, 250))]
    for n, e1, e2, g, sub, plane, lp, anch, l1, l2 in marks:
        corner = add(mul(n, 1), mul(e1, -1), mul(e2, -1))
        for e in (e1, e2):
            tip = add(corner, mul(e, 2))
            s += ln(iso.p(corner), iso.p(tip), "edge-ref")
            s += ln(iso.p(mv(Fm, corner)), iso.p(mv(Fm, tip)), "edge-def")
        # вершина угла — точка
        pc = iso.p(corner)
        s += f'<circle cx="{pc[0]:.1f}" cy="{pc[1]:.1f}" r="2.6" class="dot"/>\n'
        # выноска к грани
        s += ln(l1, l2, "leader")
        s += txt(lp[0], lp[1] - 18, f"плоскость {plane}", "cap", anch)
        s += txt(lp[0], lp[1],
                 f"γ<tspan class='sub'>{sub}</tspan> = "
                 f"{'+' if g > 0 else '−'}{num(abs(g) * 1e5)}·10⁻⁵", "lbl-s", anch)
        s += txt(lp[0], lp[1] + 16,
                 "прямой угол " + ("уменьшился" if g > 0 else "увеличился"),
                 "cap", anch)
    s += txt(320, 484, "сплошные рёбра — до деформации, штриховые — после", "cap")
    s += txt(320, 500, "углы показаны увеличенными примерно в 200 раз", "cap")
    s += '</svg>\n'
    return s
...
if __name__ == "__main__":
    ...
    K = 200.0
    Fm = [[1.0, K*GXY/2, K*GZX/2], [K*GXY/2, 1.0, K*GYZ/2], [K*GZX/2, K*GYZ/2, 1.0]]
    for nm, i, j, g in (("xOy (γxy)", 0, 1, GXY), ("yOz (γyz)", 1, 2, GYZ),
                        ("xOz (γzx)", 2, 0, GZX)):
        a = mv(Fm, UNIT[i]); b = mv(Fm, UNIT[j])
        cosang = (sum(p*q for p, q in zip(a, b))
                  / math.sqrt(sum(p*p for p in a)) / math.sqrt(sum(q*q for q in b)))
        ang = math.degrees(math.acos(cosang))
        print(f"  {nm}: на рисунке угол {ang:6.2f}°  "
              f"({'уменьшился' if ang < 90 else 'увеличился'} на "
              f"{abs(ang-90):.2f}°), γ = {g*1e5:+.2f}·10⁻⁵ -> "
