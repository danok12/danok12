...
print(f"  через главные напряжения (формула тетради): U0  = {float(U0):.5f} МДж/м³")
print(f"  через σx..τzx при G = 0,8·10⁵ (как в документе): {float(u_lin+u_sh):.5f} МДж/м³"
...
print(f"  через σx..τzx при G = E/[2(1+ν)] = {float(Gt):.1f}: "
      f"{float(u_lin + (txy**2+tyz**2+tzx**2)/(2*Gt)):.5f} МДж/м³  "
      f"-> совпадает с формулой тетради")
print(f"  U0ф по формуле тетради            = {float(U0f):.5f} МДж/м³")
print(f"  U0ф вычитанием, как в документе   = {float(u_lin+u_sh-U0v):.5f} МДж/м³"
      f"   <- в документе 0,12815")
print(f"  U0об                               = {float(U0v):.5f} МДж/м³"
